...
sheet.name = "SOPFast" # "SOPUrine" or "SOPThaw" or "SOPFast"
...
#model = lm(y ~ polym(x1, x2, degree = 1, raw = TRUE), data = df) # center and scale all predictors
#model = lm(y ~ x1 + x2 + x1*x2, data = df) # linear + interaction
model = lm(y ~ x1 + x2 + x1*x2 + I(x2*x2), data = df) # linear in x1, quadratic in x2 + interaction
