...
if (do.preprocess.flag) {
  ...
  df = df[, c("CAS Number", "Sample Name", "Area Ratio", "Temperature", "Amount", "Duration")]
  colnames(df) = c("CAS", "sample", "area", "temperature", "amount", "duration")
  
  # clean up the variables
  df[, "temperature"] = factor(df[, "temperature"], 
                               levels = c("-80C", "-20C", "4C", "25C"), 
                               labels = c("-80",  "-20",  "4",  "25"))
  ...
  df[, "duration"] = factor(df[, "duration"], 
                            levels = c("0 Days", "3 Days", "7 Days", "13 Days", "21 Days", 
                                       "35 Days", "40 Days", "171 Days", "345 Days"),
                            labels = c(0, 3, 7, 13, 21, 35, 40, 171, 345))
...
model = lm(y ~ polym(x1, x2, x3, degree = 3, raw = TRUE), data = df) # center and scale all predictors
...
if (do.3d.plots.flag) {
  ...
  for (x1 in unique(df[, "x1"])) {
    I = which(df[, "x1"] == x1)
    I = sample(I, max(length(I), 1000L))
    
    grDevices::pdf(file = paste("3d.plot.persp.x1=", x1, ".pdf", sep = ""))
    
    x2 = 0.5*(min(df[, "x2"]) + max(df[, "x2"])) + c(-0.6, 0.6)*(max(df[, "x2"]) - min(df[, "x2"]))
    x3 = 0.5*(min(df[, "x3"]) + max(df[, "x3"])) - c(-0.6, 0.6)*(max(df[, "x3"]) - min(df[, "x3"]))
    
    x2.grid = seq(x2[1], x2[2], length.out = 20L)
    x3.grid = seq(x3[1], x3[2], length.out = 20L)
    
    grid = expand.grid(x1 = x1, x2 = x2.grid, x3 = x3.grid)
    
    resp = matrix(predict(model, newdata = grid), nrow = 20L, ncol = 20L)
    
    res = plot3D::scatter3D(x = df[I, "x2"], y = df[I, "x3"], z = df[I, "y"],
                            surf = list(x = x2.grid, y = x3.grid, z = resp, facets = NA),
                            xlab = "x2 (10 log10(1 + amount))", ylab = "x3 (duration)", zlab = "y (log10(log10(area)))",
                            main = paste("Scatterplot and response surface for x1 (temperature) =", x1), color = rgb(0, 0, 1, 0.1))
    grDevices::dev.off()
    
    grDevices::pdf(file = paste("3d.plot.isoemtric.x1=", x1, ".pdf", sep = ""))
    res = scatterplot3d::scatterplot3d(x = df[I, "x2"], y = df[I, "x3"], z = df[I, "y"],
                                       xlab = "x2 (10 log10(1 + amount))", ylab = "x3 (duration)", zlab = "y (log10(log10(area)))",
                                       main = paste("Scatterplot and response surface for x1 (temperature) =", x1), color = rgb(0, 0, 1, 0.1))
    f <- function(x2, x3) {predict(model, newdata = data.frame(x1 = x1, x2 = x2, x3 = x3))}
...
raw.ref.df = data.frame(temperature =  25, amount = 1000, duration = 0)
# Alternative conditions
raw.new.df = data.frame(temperature = -80, amount = 1000, duration = 40)
